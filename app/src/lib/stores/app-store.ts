...
import {
  RepositoryWatcher,
  IRepositoryWatchChanges,
} from './helpers/repository-watcher'
import {
  IRepositorySnapshot,
...
import { enableCustomIntegration, enableWorktreeSupport } from '../feature-flag'
...
function getNetworkToastId(repository: Repository) {
  return `network-${repository.id}`
}
...
function getNetworkProgressMessage(
  ...
  switch (kind) {
    case 'push':
      return ahead > 0
        ? `Pushing ${commitCount(ahead)} to ${remote}…`
        : `Pushing to ${remote}…`
    case 'pull':
      return `Pulling latest from ${remote}…`
    case 'fetch':
      return `Fetching ${remote}…`
    default:
      return assertNever(kind, `Unknown network operation: ${kind}`)
  }
}
...
export class AppStore extends TypedBaseStore<IAppState> {
  ...
  private startRepositoryWatcher(repository: Repository) {
    const gitDirectory = repository.gitDir ?? Path.join(repository.path, '.git')
  ...
  private async onRepositoryWatchChanges(
    ...
    const state = this.repositoryStateCache.get(repository)
    if (state.isPushPullFetchInProgress || state.isCommitting) {
      return
    }

    const toastId = `external-change-${repository.id}`
    if (changes.refs) {
      this.showToast({
        id: toastId,
        kind: 'progress',
        message: 'Catching up with changes to the repository…',
      })
    }

    const before = this.getRepositorySnapshot(repository)
    ...
    const after = this.getRepositorySnapshot(repository)
    const message = describeRepositoryChange(
      before,
      after,
      await this.countNewCommits(repository, before.tipSha, after.tipSha)
    ...
    if (message === null) {
      ...
      this.showToast({ id: toastId, kind: 'info', message })
    }
  }

  private getRepositorySnapshot(repository: Repository): IRepositorySnapshot {
    const { branchesState, aheadBehind } =
      this.repositoryStateCache.get(repository)
    ...
    const tipSha =
      tip.kind === TipState.Valid
        ...
        ? tip.currentSha
        : null
    const upstreamName = branch?.upstream ?? null
    const upstream =
      upstreamName === null
        ? undefined
        : allBranches.find(
            b => b.type === BranchType.Remote && b.name === upstreamName
          )
  ...
  private async countNewCommits(
    repository: Repository,
    oldSha: string | null,
    newSha: string | null
  ): Promise<number | null> {
    if (oldSha === null || newSha === null || oldSha === newSha) {
      return null
    }

    const result = await getAheadBehind(
      repository,
      revSymmetricDifference(newSha, oldSha)
    ).catch(() => null)

    return result?.ahead ?? null
  }

  private updateNetworkToast(
    repository: Repository,
    ...
    const toastId = getNetworkToastId(repository)
    if (this.networkToastOperations.has(toastId)) {
      return
    }
    this.networkToastOperations.set(toastId, progress.kind)

    this.showToast({
      id: toastId,
      kind: 'progress',
      message: getNetworkProgressMessage(
        progress.kind,
        progress.remote,
        this.repositoryStateCache.get(repository).aheadBehind?.ahead ?? 0
      ),
    })
  }

  private async showNetworkOperationResult(
    repository: Repository,
    before: IRepositorySnapshot,
    toastId: string
  ) {
    const operation = this.networkToastOperations.get(toastId)
    this.networkToastOperations.delete(toastId)
    ...
    if (message !== null) {
      this.showToast({ id: toastId, kind: 'success', message })
    } else if (operation === 'fetch') {
      this.showToast({ id: toastId, kind: 'success', message: 'Up to date' })
    } else {
      this.dismissToast(toastId)
    }
  }
  ...
  private showToast(toast: IToast) {
    const existingTimer = this.toastTimers.get(toast.id)
    if (existingTimer !== undefined) {
      window.clearTimeout(existingTimer)
      this.toastTimers.delete(toast.id)
    }

    const others = this.toasts.filter(t => t.id !== toast.id)
    this.toasts = [...others, toast].slice(-maxVisibleToasts)

    if (toast.kind !== 'progress') {
      this.toastTimers.set(
        toast.id,
        window.setTimeout(() => this.dismissToast(toast.id), toastDurationMs)
      )
    }
  ...
  /** This shouldn't be called directly. See `Dispatcher`. */
  public _dismissToast(id: string) {
  ...
  private updatePushPullFetchProgress(
    ...
    this.updateNetworkToast(repository, pushPullFetchProgress)
    if (this.selectedRepository === repository) {
      this.emitUpdate()
    }
  }
  ...
  private async withPushPullFetch(
    ...
    await this.showNetworkOperationResult(
      repository,
      before,
      getNetworkToastId(repository)
    )
  }
